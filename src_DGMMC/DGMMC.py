...
class DGMMC_spherical(nn.Module):
    ...
    def apply_constraints(self):
        ...
        self.bandwiths_constrained = torch.clamp(self.bandwidths, min=1e-2, max=1e2)
...
class DGMMC_diagonal(nn.Module):
    ...
    def apply_constraints(self):
        ...
        self.bandwiths_constrained = torch.clamp(self.bandwidths, min=1e-2, max=3)
    ...
    def GMM_log_prob(self, value):
        ...
        #MD_element = 1/self.bandwiths_constrained * torch.matmul(diff_means_T, diff_means).squeeze()
        sigmas = torch.einsum("ni,nij->nij", 1/self.bandwiths_constrained, self.sigma_eyes)
