        ...
        for cov_type in cov_types:
            for p in P:
                for g in G:
                    ...
                    model_path = os.path.join(models_path, 'model_P_{}_G_{}_cov_{}.pt'.format(p, g, cov_type))
                    ...
                    fpath = os.path.join(results_path, 'train_P_{}_G_{}_cov_{}.csv'.format(p, g, cov_type))
                    df_tr.to_csv(fpath, sep=';')
                    
                    val = np.stack(val, axis=0)
                    df_val = pd.DataFrame(val, columns=['loss', 'acc'])
                    fpath = os.path.join(results_path, 'val_P_{}_G_{}_cov_{}.csv'.format(p, g, cov_type))
                    df_val.to_csv(fpath, sep=';')

                    te = np.vstack((test_loss, test_acc)).transpose()
                    df_test = pd.DataFrame(te, columns=['loss', 'acc'])
                    fpath = os.path.join(results_path, 'test_P_{}_G_{}_cov_{}.csv'.format(p, g, cov_type))
                    ...
                    fpath = os.path.join(results_path, 'Features_P_{}_G_{}_cov_{}.csv'.format(p, g, cov_type))
